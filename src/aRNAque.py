...
def run(number_of_generation,pop, ftool,landscape, p_n, p_c,msf,sm='NED',EDg=0, constraints=None, c=None,lb=True) :
    ...
    tic = time.time()
    best_pop, Mgen = simple_EA(landscape,number_of_generation,init_pop, sm,ftool, pos, p_n, p_c, msf=msf, constraints=constraints,c=c)
...
def main() :
    ...
    ppservers = ()
    job_server = pp.Server(number_of_run, ppservers=ppservers)
    pops = []
    pop = init(pop_size, landscape, constraints=None)
    for i in range(number_of_run) :
        pops.append(pop)

    run(number_of_generation,pops[0], args.folding_tool,landscape, p_n, p_c,msf,sm,EDg,constraints,args.c,args.lb)
