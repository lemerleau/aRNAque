...
def simple_EA(landscape, number_of_generation, init_pop, sm, ftool,pos,p_n, p_c, msf=10, constraints=None,c=None,mu=None, lb=False) :
    """
    ...
    while (n<number_of_generation) and (max_found < msf):
        ...
        dist = utility.gen_point_mutation_dist(population_size,pos,c,mu)
...
def run(number_of_generation,pop, ftool,landscape, p_n, p_c,msf,sm='NED',EDg=0, constraints=None, c=None,mu=None,lb=False) :
    ...
    tic = time.time()
    best_pop, Mgen = simple_EA(landscape,number_of_generation,init_pop, sm,ftool, pos, p_n, p_c, msf=msf, constraints=constraints,c=c, mu=mu)
...
def parseArguments() :
    ...
    parser.add_argument('-mu', type=float,default=None, help="binomial mutation rate" )
...
def main() :
    ...
    jobs = [(task , job_server.submit(run, (number_of_generation,pops[task], args.folding_tool,landscape, p_n, p_c,msf,sm,EDg,constraints,args.c,args.mu,args.lb), (mutate,ED_refinement,elite, select, simple_EA),
